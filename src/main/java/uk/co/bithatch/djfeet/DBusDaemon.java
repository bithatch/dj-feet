...
public class DBusDaemon extends Thread implements Closeable {
    ...
    public static void saveFile(String data, String file, boolean insecure) throws IOException {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            for (int i = 0; i < args.length; i++) {
                if ("--help".equals(args[i]) || "-h".equals(args[i])) {
                    ...
                    version();
                } else if ("--insecure".equals(args[i]) || "-I".equals(args[i])) {
                	insecure = true;
                } else if ("--listen".equals(args[i]) || "-l".equals(args[i])) {
                    addr = args[++i];
                } else if ("--authmode".equals(args[i]) || "-l".equals(args[i])) {
                	authmode = SaslAuthMode.valueOf(args[++i]);
        ...
        if (printaddress) {
            System.out.println(addr.replace("listen=true,", ""));
        }
